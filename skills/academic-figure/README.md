...
`FontProperties`/`axes.unicode_minus=False`/`fig.canvas.draw()`，R 使用
`systemfonts` 等价检测和 Cairo 渲染；缺字、乱码或渲染视觉 QA 失败时不进入正式图目录。
...
| **语义丰富 > 无意义堆色** | 单个 Figure 使用 2-4 个语义主色 + 1 个强调色；全篇可随证据类型变化配色角色，禁用 matplotlib/ggplot 默认色板 |
...
```text
    ...
    │   ├── compose.py                 ← 多面板排版引擎
    │   ├── eval_runner.py             ← 全量资产审计（39 类资产自动扫描）
