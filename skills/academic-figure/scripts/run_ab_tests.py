...
def run_all():
    ...
    s1_skill_passes.append(pca_r.exists())
    s1_skill_details.append(f"Asset exists: {pca_r.exists()}")
    ...
    s1_skill_passes.append('type="cairo"' in py_src or "type='cairo'" in py_src)
    s1_skill_details.append("PNG cairo rule in compose.py")
    ...
    s1_skill_passes.append("#2166AC" in color_src and "#B2182B" in color_src)
    s1_skill_details.append("CNS palette in color-palettes.md")
    ...
    s1_skill_passes.append("Arial" in typo_src)
    s1_skill_details.append("Arial font in typography.md")
    ...
    s1_skill_passes.append("cairo_pdf" in export_src)
    s1_skill_details.append("cairo_pdf vector export")

    # Check 300dpi
    s1_skill_passes.append("300" in export_src or "dpi = 300" in export_src.lower())
    s1_skill_details.append("300dpi rule")

    report["scenarios"]["S1_pca"] = {
        "academic-figure-skill": {
            "passed": sum(s1_skill_passes), "total": len(s1_skill_passes),
            "pass_rate": sum(s1_skill_passes) / len(s1_skill_passes),
            "checks": s1_skill_details,
        },
        "baseline": {
