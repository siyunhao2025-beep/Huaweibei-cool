...
For Chinese result figures, detect the installed font with `systemfonts` (or
`sysfonts`/`showtext`) and pass the resolved family explicitly to ggplot2 or
ComplexHeatmap. Do not silently accept an English-only fallback. The font
smoke test must include Chinese labels, units, RMSE, negative values, Greek
