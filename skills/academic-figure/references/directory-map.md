...
| **TrajectoryProjection** | 三维轨迹, 轨道投影, 路径投影, 3D trajectory, orbit projection, spatial path | 3D trajectory with inspectable 2D projection/inset |
| **ParameterSweep** | 参数扫描, 理论边界, 响应面, parameter sweep, response surface, feasibility boundary | Sweep heatmap/contour with boundary and optimum |
| **ResidualDiagnostics** | 残差诊断, 守恒量残差, 误差结构, invariant residual, residual diagnostics | Signed residual sequence with distribution/invariant reference |
| **MonteCarloRecovery** | 蒙特卡洛恢复, 注入恢复, 重复试验, injection recovery, Monte Carlo recovery | Truth-recovery density, bias and coverage evidence |
| **ConvergenceBand** | 收敛曲线, 多种子收敛, 误差带, convergence band, feasibility history | Median convergence with uncertainty and violation history |
| **SensitivityTornado** | 灵敏度龙卷风, 参数影响排序, tornado sensitivity, local sensitivity | Ranked positive/negative parameter effects with intervals |
| **ScenarioHeatmap** | 场景热图, 消融热图, 开关式消融, scenario heatmap, ablation heatmap | Scenario/component response matrix with annotated optimum |
| **ContributionWaterfall** | 贡献瀑布图, 时延分解, 成本分解, signed waterfall, contribution decomposition | Signed component contributions and cumulative total |
| **PhaseProfile** | 相位栅格, 周期轮廓, 折叠图, phase raster, folded profile | Phase-cycle raster paired with folded periodic profile |
| **ResourceDesign** | 观测时长设计, 资源需求曲线, 预算拐点, resource design, observation time | Resource-performance curve with feasible region and knee point |
