...
Academic Figure Skill uses a vivid but journal-safe semantic palette rather than default matplotlib, ggplot2, seaborn, Excel, Scanpy, or rainbow palettes. The target look is high contrast, print-safe, colorblind-aware, and semantically assigned. A paper may use varied palette roles across distinct Figure families; a single Figure still limits colors to what its data semantics require.
