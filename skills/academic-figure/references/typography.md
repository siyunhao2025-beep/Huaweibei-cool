...
## Chinese display layer (project requirement)

For ordinary Huawei Cup result figures, visible titles, axis labels, legends,
annotations, panel descriptions, captions and statistical explanations use
Chinese by default. Keep source data fields unchanged and map labels only at
the rendering layer. Before plotting, resolve an installed font with
`scripts/chinese_fonts.py`; use its `FontProperties(fname=...)` for Chinese
text, set `axes.unicode_minus=False`, and call `fig.canvas.draw()` before any
export. A missing font or failed glyph smoke test leaves the Figure Plan
status **incomplete**.
