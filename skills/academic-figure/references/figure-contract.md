...
If unsure and panel information density differs, default to `asymmetric mixed-modality`; use `quantitative grid` only when all panels carry comparable scientific weight. This matches the Hub's canonical `asymmetric_mixed` default and avoids mechanically repeating equal `1×2` grids.
