...
def draw_tornado(df, prop):
    ...
    for yi, value, low_value, high_value in zip(y, ordered.effect, ordered.low, ordered.high):
        x_text = high_value + 0.018 if value > 0 else low_value - 0.018
        ax.text(x_text, yi, f"{value:+.2f}", va="center", ha="left" if value > 0 else "right", fontsize=7)
...
def draw_scenario(df, prop):
    ...
    best = np.unravel_index(np.argmax(pivot.to_numpy()), pivot.shape)
    ax.add_patch(plt.Rectangle((best[1]-0.48, best[0]-0.48), 0.96, 0.96, fill=False, edgecolor="#F1A340", linewidth=2.0))
