...
METHOD_SYNONYMS = [
    ...
    # 禁忌搜索：仅认显式中文术语或 tabu/taboo search，(?!lar) 排除 tabular_CPD 子串；
    # 不用裸 \bts\b（TS 可能指时间序列/运输等），避免泛化污染。
    (r"禁忌搜索|禁忌表|禁忌对象|禁忌长度|tabu(?:\s*search)?(?!lar)|taboo(?:\s*search)?", "禁忌搜索(TS)"),
...
def main_stats():
    ...
    A("- “禁忌搜索(TS)”经 Wave4-D 清洗：泛化标签“禁忌搜索/启发式”已改为“启发式优化”，仅保留 4 篇正文真实现禁忌搜索的论文；同义词表同时收紧（排除 tabular_CPD 子串、停用裸 \\bts\\b）。")
    ...
    A("- 奖级经 Wave4-D 文本扫描：一等奖 1、数模之星提名奖 12（2021 专集，目录证据）、待确认 716；正文极少自述奖级，按奖级做统计时样本受限。")
