...
## Unreleased — 2026-09-23（第二十三届模板融合、工程加固与诚实验证）

- 2026-09-26 重新核验用户桌面的第二十三届社区 LaTeX 包：PDF 仍为 A4 共 12 页，`main.tex` 未变化；PDF、`gmcmthesis.cls` 与 `figures/title.pdf` 已更新，清单同步刷新字节数和 SHA-256。新版 `title.pdf` 已是“第二十三届”且会被社区类文件调用，但因源包仍无明确再分发许可，继续只作本机参考，不复制或接入生产模板。
- 新增 `章节模板/任务映射与总体分析模板.tex`，把“题面口径—任务映射—成功条件—总体证据链—复杂度/验证安排—共享数据审计”落成可编译的自适应骨架；不固定问题数，不预设“结构识别—预测—优化”题型顺序，简单题删除不适用章节。同步加入仓库契约测试。
- 修复 CI 与新增 PDF 集成测试的环境错配：Ubuntu 显式安装 XeLaTeX、CTeX、`ulem`、`siunitx`、Fandol、TeX Gyre 正文/数学字体及所需 LaTeX 包，并在测试前逐项检查关键样式与字体；标题字体回退改用 TeX 树内 OTF 文件名，避免最小 Linux runner 未把 Fandol 注册进 fontconfig。三轮修正后本机全量回归更新为 375 passed、0 skipped/xfail/xpass。
- 将“重跑题目并覆盖错误 PDF”固化为第 25 条硬规则：先锁定原始输入/权威入口并备份旧件，失效模型派生缓存，按依赖图全量重跑求解—验证—图表—论文链；新图表同步到 TeX 真实读取目录后，执行双遍编译、隔离目录干净构建、文本/渲染指纹比对和全页视觉 QA。所有门禁通过后才原子替换全部交付别名，重建并解压复编源码/Overleaf 与复现包，更新页数、审计报告和 SHA-256 清单。明确原始 PDF 字节哈希不单独证明内容等价，空白身份或历史题/当届格式不一致仍只允许内部审阅。
- 按用户最新裁决新增“一页摘要页”内部硬规则：物理第 2 页完整容纳匿名题目、摘要与 3--6 个关键词，正文从物理第 3 页开始；题目优先单行，必要时最多两行且最终宽度比不低于 0.85。模板居中处理多行题目、关键词续行采用悬挂缩进并禁止词内断行；源码审计拒绝缩字号/压行距/缩放/负间距/手工分页，PDF 审计按真实页面与 bbox 核对。官方原文仍是摘要“一般不超过两页”；单页是跨度上限而非最低字数，不允许为“写满”灌水。
- 将复杂多问论文的 F01 从“六层简单框图”升级为**论文专属完整框架图**：接入 `paper-framework-figure-studio-pro`，要求真实数据/结果锚点、各问内部机制、跨问关系、真实存在的验证/选择控制（若有）与独立测试边界；YAML 路线图降为语义草图、低复杂度充分表达或后备。
- 新增总流程图入稿六联门禁：正式源图、LaTeX 绑定、图前引用、自包含图注、图后解释和编译后 PDF 可见缺一不可；明确禁止生成了正式图却让旧简单框图继续占据论文 F01。
- 新增中文复杂 F01 文本合同与 `audit_framework_figure.py`：中文自然语言优先，技术符号/缩写按证据保留；矢量图按最终有效字号、栅格图按原生像素和插宽后的物理墨迹高度审查，禁止虚报 pt 或伪放大。审计会展开受控 LaTeX 图片宏并精确解析目标路径；成品 PDF 中的位图按解码像素哈希核验，矢量图按页内坐标框、前景覆盖和区域渲染核验，避免同尺寸假图或空白区域伪通过。
- 撤销“所有路线图都必须画反馈回路”的错误硬编码：改由 `metadata.feedback_expected` 声明真实控制关系；独立测试、最终评价与只读审计不得反向反馈，避免为了图面完整而虚构箭头。
- 按用户提供的 12 页内容参考补齐逐标题写作映射；所有章节模板加入“本节问题—输入/证据—方法/定义—结果—解释/边界—接口”注释卡，避免标题空壳。
- 将逐图叙述升级为“图前定位—图注解码—证据读数—原因/机制—论文含义—边界/转折”合同：每张图都要回答怎么看、为什么、意味着什么和哪里失效；复合图覆盖关键面板，流程图解释模块/箭头/隔离，相关性不得冒充因果。
- 新增“大图不背靠背”的图间叙事门禁：同一小节允许多张顶层 Figure，但相邻大图之间至少有一段、复杂时两段实质正文，完成上一图证据收束、尚存缺口与下一图验证目的；`audit_tex.py` 会拒绝连续堆图、仅靠标题/分页隔开或空泛“下面给出下一图”，复合图子面板不另计。
- 将“参考文献另起一页、附录另起一页”固化为生产排版合同：LaTeX 入口前统一 `\clearpage`，Word 标题启用段前分页；源码审计会阻止手工改稿时丢失分页，同时明确该规则不冒充官方原文、不用于凑页。
- 将计算型论文的代码式伪代码固化为附录合同：必须由真实运行链提炼并覆盖输入、配置/随机性、核心计算、验证和导出，逐步回链脚本/函数与结果文件；完整源码留在复现包，纯解析题只可用有理由的显式不适用。`audit_tex.py` 新增适用性、代码环境和可追溯性三层门禁，拒绝空壳代码、占位行和未实例化模板。
- 将编号问题的开篇叙事固化为“回溯—路线—选择理由—结果交付”合同：一级问题标题后、首个小标题/公式/图表前须先给 2--4 句本题专属导读，再循序进入模型、求解、结果与验证。`audit_tex.py` 支持中文/阿拉伯编号、Q/Question 标题和嵌套 `\input`，会拒绝空泛“本节将”、注释伪装、技术内容后补导读及附录 Q2 标题误报。
- 新增“主要符号说明”内部硬规则：正式论文在基本假设/总体分析之后、第一问推导和首个模型公式之前设置且只设置一张主符号三线表；表内只收全局核心符号，单位或取值域与正文逐项一致。`audit_tex.py` 与 `paper_checklist.py` 共用同一结构化分析器，拒绝注释伪装、横线分散、空表、多表、竖线网格、缺题注/标签/前置引用，以及 `resizebox/scalebox` 或小字号挤压；Word 派生链同步保留表题和真实三线边框，`audit_docx.py` 会拒绝 Table Grid 或竖线伪装；模板、quickstart、写作与终审文档同步落地，并明确这不是官方明文要求。
- 区分“可继续编辑的空封皮脚手架”和“可正式提交的已填写封皮”：`audit_tex.py --require-filled-cover` 与 `submission_audit.py --cover-policy required` 会核对学校、参赛队号及三名队员姓名均为非空、非占位值；PDF 路径按官方固定封皮的字段区域阻止空白页伪绿，四个 logo 仍保留人工终审。
- 复核后确认仓库原 `研赛论文Word标准模板.docx` 是旧 v2.1 社区写作稿而非 2026 官方附件 3，且含目录、占位正文和错误页边距；当前树将其撤下，Word 派生器改由用户传入有权使用的本地模板，相关清理与审计能力保留。同步撤下与源码脱节的预打包 Overleaf ZIP，并纠正第三方模板“已有明确许可”的错误表述；MIT 只覆盖贡献者有权许可的原创部分，第三方公开再分发仍须取得授权。
- 加固视觉与 PDF 审计器：能正确统计 Figure 包装宏、识别图前/图注/图后闭环，保留 `95\%` 等转义百分号；遇到只写 ID 的旧式表格账本时输出明确失败项而不是崩溃；中文字体 CMap 损坏时可接收经人工核验且页界合法的显式物理页码，并标注为 `declared`。
...
- 当前源码回归为 375 passed、0 skipped/xfail/xpass；本轮未把仓库改动同步到本机安装副本，不能把源码通过冒充安装副本已更新。此前 doctor 10/10 PASS、skill 快速校验、4 页正式模板示例和 4 页 quickstart 玩具稿的验证记录继续保留；quickstart 已明确不承担正式格式认证，客观不适用项不再伪装为通过。F2023 工作稿加入真实伪代码、四问开篇导读、主符号三线表、中文版论文级 F01 与 20 张大图之间的叙事桥后，临时双遍编译为 57 页；20 张顶层 Figure、19 个图间承接段均通过源码门禁，高风险相邻图页经成品 PDF 复核无连排、越界、遮挡或未解析引用。真实 F01 另通过 LaTeX 宏展开、位图像素哈希与编译页绑定审计。
...
- 统一西文排版：官方 Word 模板中 ASCII/HAnsi 的 Times New Roman 设置已映射到 LaTeX；本地使用 Times New Roman，Overleaf 合法回退为 TeX Gyre Termes，公式改用 TeX Gyre Termes Math。按用户内部裁决，代码环境中的西文也使用 Times New Roman/TeX Gyre Termes，并保留逐行结构、缩进和换行，不再使用 Courier 等字体例外。
