...
- 修复 CI 与新增 PDF 集成测试的环境错配：Ubuntu 安装 XeLaTeX、CTeX、中文字体和所需 LaTeX 包，并在测试前检查 `xelatex` 与 `ctexart.cls`；本机全量回归更新为 374 passed、0 skipped/xfail/xpass。
...
- 当前源码回归为 374 passed、0 skipped/xfail/xpass；本轮未把仓库改动同步到本机安装副本，不能把源码通过冒充安装副本已更新。此前 doctor 10/10 PASS、skill 快速校验、4 页正式模板示例和 4 页 quickstart 玩具稿的验证记录继续保留；quickstart 已明确不承担正式格式认证，客观不适用项不再伪装为通过。F2023 工作稿加入真实伪代码、四问开篇导读、主符号三线表、中文版论文级 F01 与 20 张大图之间的叙事桥后，临时双遍编译为 57 页；20 张顶层 Figure、19 个图间承接段均通过源码门禁，高风险相邻图页经成品 PDF 复核无连排、越界、遮挡或未解析引用。真实 F01 另通过 LaTeX 宏展开、位图像素哈希与编译页绑定审计。
