...
- **3 处转录存疑（待用户确认，已在条目内标注"待用户确认"）**：
  - **M14**（预测模型评估）：原图此行被水印遮挡，按"分类模型评估（M11）"同构条目补全。
  - **I02**（改进可否分点）：原图两条互相矛盾，理解为"以整段为主、分点不得跑题"。
  - **V05**（误差检验）：原图字迹疑似"优先使用五折交叉验证"。
- **与官方规范的冲突（已如实记录，未改模板）**：
  自检表 **F03** 要求二三级标题"小四号**黑体**"；但华为杯官方 `华为杯_论文章节规范.md` §2.3(1)
  明确为"小四号**宋体**"，且禁止改 `gmcmthesis.cls`；`gmcmthesis.cls` 与 Word 模板现状均为宋体小四。
  按"当届官方规范优先于第三方自检表"原则，**保持宋体小四、不改模板**，冲突记录在
  `modules/paper-writing.md` §6.1，待用户最终裁决。
