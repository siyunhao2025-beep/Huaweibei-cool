...
> **诚实声明**：与 §5 那轮"只喂标题+task_types"的留出盲检不同，本轮 problem_text 是我依据 tracks 档案与简卡**整理写就**的题面描述，本身已含区分度词汇。因此 93%→100% 衡量的是**规则回归一致性**，不是对全新真题的泛化能力；真正的泛化证据仍是 §5 的 40%（10 篇留出、仅元数据）。
...
3. **下调 simulation 基置信度 0.8→0.78**：使 R05 mechanism（0.85）在"机理/状态机在场"时天然压过 simulation，即"仿真仅作验证手段"不单独判 simulation；R08 加 `博弈推演/器件仿真/器件级仿真/栅格`，纯仿真题靠多词加权（+0.05/词）仍能取胜。
4. **补 signal 通信词**：R06 加 `SINR/链路/子载波/调制/解调/载波`，修 `2025_B`。
5. **纠偏**：一度把"反演"加进 prediction，导致 `2025_D`（大气物理机理）被牵走，随即移除——物理反演量估计不归时序预测。

### 调优后（2026-09-22 回归修正）：top1 30/30 = 100%，known_trap 3/3 = 100%
...
| spatial-graph | 0 | 0 | 0 | 0 | 0 | 0 | 2 | 0 | 2 |
| simulation | 0 | 0 | 0 | 0 | 0 | 0 | 0 | 2 | 2 |

### 误差分析（如实）

- **`2017_B` 已修正**：R08 增加“器件级仿真”强范式词，使“建立器件级仿真模型”压过后续参数优化方法词；不再用 `xfail` 豁免。
- **跨原型题天然歧义未强求单解**：如 2022_E（机理+预测）、2024_D（预测+空间+评价），本题只对 top1 主原型判分，辅原型记在 `golden_secondary`。
- **阈值不做虚高**：当前回归集实测 100%，pytest 断言阈值仍取 90%（≥27/30），留余量防规则微调抖动；已知陷阱题单独硬断言 3/3。
