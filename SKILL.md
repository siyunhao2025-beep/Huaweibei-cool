...
15. **复杂多问论文的图1必须是论文级总流程图**：若题目包含多个相互关联的小问、数据链、训练/验证/测试边界或跨问依赖，F01 不得用“输入—模型—输出”几只空框草率代替。先按 `paper-framework-figure-studio-pro` 的完整论文框架路线，从题面、数据审计、模型/求解脚本、结果文件和证据账本构造论文专属总图；图中至少可辨认真实输入、各问核心机制、跨问数据/结果流、真实存在的验证/选择控制（若有）、独立测试边界和最终证据输出。真实数据缩略图、结果小面板和必要数学符号可用于降低阅读成本，但不得虚构模块、箭头、变量或收益。`render_roadmap.py` 只承担结构草图、低复杂度题的充分表达或正式图的语义底稿；它不能自动等价于复杂论文的最终 F01。详见 `technical-roadmap.md`。
16. **生成不等于入稿，图文必须闭环**：正式总流程图只有同时满足“选定高清源图—写入 LaTeX—正文在图前引用—自包含图注—图后解释读数/关系/边界—编译后 PDF 肉眼可见”才算完成；任一环缺失即回退修复。每个二、三级标题下先写清该节回答的问题、输入/证据、方法、结果和与下节的接口；每张结果图按“图前定位—图注解码—图后证据解释—边界/转折”叙述，禁止只放图、只写‘如图所示’或让图注替代核心论证。详见 `paper-writing.md` 与 `figures-interface.md`。
