...
1. 从赛前、选题、建模、写作、终审五个阶段中判断当前阶段；不能判断时只问一个阶段问题。
2. 只索取当前推进所必需的材料，最多三类，例如题面、数据、现有代码/论文；不要求用户一次交齐全流程文件。
3. 返回“当前阶段、所需材料、下一步三件事、官方风险提醒”四项短结果。
4. 2026 终审或上传场景必须提醒：正式 PDF 首页是含团队信息的官方封皮，第二页起匿名；不能上传纯匿名摘要稿代替正式 PDF。
...
| P4 | 论文初稿 | 摘要/正文/结论 | 章节齐、摘要一般不超过2页、自检表文件就位 |
| P5 | 打磨审校 | 终稿、封皮后匿名化、引用 | audit 脚本 PASS |
