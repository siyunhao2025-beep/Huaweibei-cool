...
| P3 | 求解 | 代码、结果、验证 | 可复现+≥1验证；有候选改进时已完成策略选择 |
...
13. **50+ 条件目标与反凑页**：50+ 指完整物理 PDF 的用户内部目标，不是官方门槛。只有每问证据链闭合且新增页面具有独立信息增量时才追求；未达到先运行证据缺口审计，若没有真实缺口则接受较短稿并明确说明。禁止以套话、重复图表、无效复杂度、放大字号/图表、强制分页或无关附录凑页。图表数量服从 `figures-interface.md` 的证据组合与去重门槛。
