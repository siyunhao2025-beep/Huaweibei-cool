...
## 页数结论与内部目标（核对至 2026-09-21）

- 当届官方材料只明确摘要篇幅**一般不超过两页**；没有给出正文最低页数、正文上限或全文总页数上限。
- 本 skill 另设“完整物理 PDF 50+ 页”的**用户内部条件目标**：只在每问“任务—数据—Baseline—模型—结果—验证—结论”闭环、图表各有独立信息增量时追求；它不是官方要求，也不保证奖项。
- 低于 50 页先补真实证据缺口；若关键推导、对标、诊断、不确定性、鲁棒性、运行成本、边界和复现信息均已充分，则保留较短稿并说明原因。禁止套话、重复图表、无效模型、放大图表、强制分页和无关附录。
...
scripts/build_latex.py 从 TeX-only manifest 生成带正式封皮结构的主文件；scripts/audit_tex.py 检查输入顺序、无目录、封皮后匿名、页式和格式覆盖；scripts/audit_paper.py 在完整 PDF 少于内部 50 页目标时只发出证据缺口警告，不会把它误判为官方违规；scripts/build_docx.py 与 scripts/audit_docx.py 提供可选 Word 派生链。最终版式仍须以 PDF 视觉检查为准。
